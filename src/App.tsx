...
const TARGET_FILES = [
  'watch_accelerometer.csv','watch_gravity.csv','watch_gyroscope.csv',
  'watch_heart_rate.csv','watch_light.csv','watch_ppg_green.csv','response.csv',
] as const
...
async function* dirEntries(dir: DirHandle): AsyncGenerator<[string, FileSystemHandle]> {
  ...
  if (it) { // @ts-ignore
    for await (const e of it) yield e as [string, FileSystemHandle]; return
  }
...
function guessNumericKeys(rows: any[], prefer = ['value','data','x','y','z','X','Y','Z']) {
  if (!rows?.length) return []
  const sample = rows[0]
  const ks = Object.keys(sample)
  const nums = ks.filter(k => typeof sample[k] === 'number')
  nums.sort((a,b)=> (prefer.indexOf(a)===-1?999:prefer.indexOf(a))-(prefer.indexOf(b)===-1?999:prefer.indexOf(b)))
  return nums
}
function guessTimeKey(rows: any[], prefer=['timestamp','time','date','datetime','createdAt']) {
  if (!rows?.length) return undefined
  const ks = Object.keys(rows[0]); for (const p of prefer) if (ks.includes(p)) return p; return undefined
}
function prettyName(name: string) {
  const map: Record<string,string> = {
    watch_accelerometer:'가속도계', watch_gravity:'중력', watch_gyroscope:'자이로',
    watch_heart_rate:'심박', watch_light:'조도', watch_ppg_green:'PPG(녹색)', response:'설문 응답',
  }
...
export default function App() {
  ...
  const pickRoot = async () => {
    setError(null)
    try { const dir = await (window as any).showDirectoryPicker(); setRoot(dir) }
    catch (e: any) { if (e?.name !== 'AbortError') setError('폴더 선택이 취소되었거나 지원되지 않습니다.') }
  }

  /* 스캔 */
  useEffect(() => { (async () => {
    if (!root) return
    setSelectedUid(null); setSelectedCsv(null); setRows(null); setError(null)
    ...
    setScanTotal(list.length); setScanDone(0)

    if (fastScan) {
      const limit = pLimit(16)
      const scanned = await Promise.all(list.map(p => limit(async () => {
        let count = 0
  ...
  const ensureDetailFor = async (uid: string) => {
    ...
    if (detailCache.has(uid)) {
      if (!p.files) setParticipants(ps => ps.map(pp => pp.uid===uid ? { ...pp, files: detailCache.get(uid)! } : pp))
      return
    }
    const files: CsvFileInfo[] = []
    for await (const [fname, fhandle] of dirEntries(p.handle)) {
    ...
    setParticipants(ps => ps.map(pp => pp.uid===uid ? { ...pp, files, quickCount: files.length } : pp))
  }

  const loadCsv = async (f: CsvFileInfo) => {
    setError(null); setSelectedCsv(f); setRows(null); setLoading(true)
    rightTopRef.current?.scrollIntoView({ behavior:'smooth', block:'start' })
    try { const data = await parseCsvFile(f.handle); setRows(data) }
    catch (e: any) { setError(`CSV 읽기 오류: ${String(e?.message || e)}`) }
  ...
  const summary = useMemo(()=>({ total: participants.length, withAny: participants.filter(p=>p.quickCount>0).length }),[participants])
        ...
        <div className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-1 text-xs text-stone-700">
              <input type="checkbox" className="accent-green-600" checked={fastScan} onChange={e=>setFastScan(e.target.checked)} />
              빠른 스캔 모드
            </label>
        ...
        <section className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div ref={rightTopRef} className="rounded-2xl border border-[#E6DCA8] bg-[#FFF5D9] p-5 shadow-sm">
            ...
            {selectedUid ? (() => {
              ...
              if (!p) return null
              if (!p.files) {
                return <div className="rounded-md border border-[#E2D8A1] bg-[#FFF2CC] px-3 py-2 text-sm text-stone-700">상세 정보 불러오는 중…</div>
              }
              return (
                        ...
                        <LineChart data={rows.slice(0, 5000)}>
                          ...
                          {valueKeys.slice(0, 3).map((k, idx) => (
                            <Line key={k} type="monotone" dataKey={k}
                              stroke={seriesColors[idx % seriesColors.length]} dot={false} strokeWidth={1.6} />
                          ))}
                        </LineChart>
                      ) : (
